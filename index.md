...
(package ID: `com.leafkeep.plants`).
...
(plant name, cost, watering interval, next watering date, location,
category, and whether it is toxic to pets) is stored in a local
...
that processing — LeafKeep never sees or receives your payment
details.
...
than the one a plant's cost was entered in, the app fetches current
...
convert amounts between currencies for display; no plant data or
other personal data is included in or derived from this request, only
the currency codes needed to look up a rate. The request is sent
...
| Permission | Purpose |
|---|---|
| **Notifications** (`android.permission.POST_NOTIFICATIONS`) | Used to remind you when a plant needs watering. Reminders are scheduled entirely on your device. On Android 13+, this requires your explicit consent, which the OS will ask for. |
...
The app does not request access to your contacts, location services,
camera, microphone, photos, or other sensitive device data. (Note:
the in-app "Location" field for grouping plants, e.g. "Living Room",
is free-text you type yourself — it has no connection to the device's
...
Plant data you enter is stored in the app's local database on your
device and remains there until you edit or delete it, or delete the
app's local data. The developer does not operate its own server and
does not receive or disclose this data to third parties.

If Android Auto Backup is enabled for LeafKeep, this local database
may also be included in a backup associated with your Google Account,
...
Since LeafKeep does not store or transmit your plant data on the
developer's side, most of these rights are already effectively in
your own hands: you can view, edit, or delete your plant data
...
- Processing of the plant data you enter (Section 3.1) is based on
  Art. 6(1)(b) GDPR, insofar as it is necessary to provide the plant
  tracking and watering reminder functionality you requested, or
  otherwise on Art. 6(1)(f) GDPR (legitimate interest in operating
...
LeafKeep lets you track care for houseplants by name (for example
Monstera, Pothos, or similar common plant names). Any names referenced
or selectable within the app are common names and are not trademarks
of the developer. Company, product, and service names used in the
app are for identification purposes only and do not imply any
affiliation, sponsorship, or endorsement. You are always free to
enter a custom plant name instead of choosing from the predefined
list.
